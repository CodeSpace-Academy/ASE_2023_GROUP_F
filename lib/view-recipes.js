...
export async function getViewRecipes(startIndex = 0, pageSize = 50, numInstructions = 2) {
	const filter = {instructions: {$size: numInstructions}}

	try {
		const skipCount = Math.floor(startIndex / pageSize);
		const newLimit = pageSize + skipCount * pageSize;
		
		const response = await fetch(`/api/recipes?limit=${newLimit}&filter=${JSON.stringify(filter)}`);
		...
		return { recipes, totalRecipes: data.totalRecipes };
	} catch (error) {
