...
const SearchBar = (props) => {
  ...
  const [submittingLongQuery, setSubmittingLongQuery] = useState(false);
  ...
  useEffect(() => {
    ...
    const debouncedSubmitLongQuery = debounce(async () => {
      setSubmittingLongQuery(true);

      debouncedApplyFilters(searchTerm);
    }, 500);

    debouncedSubmitLongQuery();

    return () => {
      debouncedApplyFilters.cancel();
      debouncedSubmitLongQuery.cancel();
    };
  }, [searchTerm, filters, sortOption, applyFilters, setSubmittingLongQuery]);
