...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (sort === 'prep ASC') querySort.prep = 1;
      else if (sort === 'prep DESC') querySort.prep = -1;

      if (sort === 'cook ASC') querySort.cook = 1;
      else if (sort === 'cook DESC') querySort.cook = -1;

      if (sort === 'date ASC') querySort.published = 1;
      else if (sort === 'date DESC') querySort.published = -1;
      ...
      if (JSON.stringify(querySort) !== '{}') agg.push({ $sort: querySort });

      // Add aggregation stage for filter criteria
      if (JSON.stringify(queryFilter) !== '{}') agg.push({ $match: queryFilter });
